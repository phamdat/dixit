...
/// - 4 new Swipe : UpLeft, UpRight, DownLeft , DownRight
/// - New event : On_Pinch
/// - New method GetCurrentPickedObject on gesture class
/// - New mehode  getfirst sur easytouch et gesture
/// - New event : On_OverUIElement, On_UIElementTouchUp
/// - tmpArray of touches is now cahced to prevent garbage collector.
/// - new mode for detection object and ui element for two finger gesture.
using UnityEngine;
...
public class EasyTouch : MonoBehaviour {
	...
	/// </summary>
	public enum SwipeType{ None, Left, Right, Up, Down, UpLeft, UpRight, DownLeft, DownRight,Other};
	...
	private void OneFinger(int fingerIndex){
		...
		if ( fingers[fingerIndex].gesture==GestureType.None){
			...
			// we notify a touch
			CreateGesture(fingerIndex, EventName.On_TouchStart,fingers[fingerIndex], SwipeType.None,0,Vector2.zero);
		}
		...
		if (fingers[fingerIndex].phase != TouchPhase.Ended && fingers[fingerIndex].phase != TouchPhase.Canceled){
			...
			if (fingers[fingerIndex].phase == TouchPhase.Stationary  && 
				...
				CreateGesture(fingerIndex, EventName.On_LongTapStart,fingers[fingerIndex], SwipeType.None,0,Vector2.zero);	
			}
				...
				|| ((fingers[fingerIndex].gesture == GestureType.Acquisition ||fingers[fingerIndex].gesture == GestureType.LongTap) && (FingerInTolerance(fingers[fingerIndex])==false) && gesturePriority == GesturePriority.Tap ))
			{
				...
				if (fingers[fingerIndex].gesture == GestureType.LongTap){
					fingers[fingerIndex].gesture = GestureType.Cancel;
					CreateGesture(fingerIndex, EventName.On_LongTapEnd,fingers[fingerIndex],SwipeType.None,0,Vector2.zero);
					// Init the touch to start
					fingers[fingerIndex].gesture=GestureType.Acquisition;	
				}
				else{
					fingers[fingerIndex].oldSwipeType = SwipeType.None;
					// If an object is selected we drag
					if (fingers[fingerIndex].pickedObject){
						fingers[fingerIndex].gesture = GestureType.Drag;
						CreateGesture(fingerIndex, EventName.On_DragStart,fingers[fingerIndex],SwipeType.None,0, Vector2.zero);
					}

					// If not swipe
					else{
						fingers[fingerIndex].gesture = GestureType.Swipe;
						CreateGesture(fingerIndex, EventName.On_SwipeStart,fingers[fingerIndex], SwipeType.None,0,Vector2.zero);
					}
				}
			}
			...
			switch (fingers[fingerIndex].gesture){
				...
				case GestureType.Cancel:
					CreateGesture(fingerIndex, EventName.On_Cancel,fingers[fingerIndex],SwipeType.None,0,Vector2.zero);
					break;

			}
				
			CreateGesture( fingerIndex, EventName.On_TouchUp,fingers[fingerIndex], SwipeType.None,0,Vector2.zero);
	...
	IEnumerator SingleOrDouble(int fingerIndex){
		...
		if (singleDoubleTap[fingerIndex].count <2){
			CreateGesture( fingerIndex, EventName.On_SimpleTap,singleDoubleTap[fingerIndex].finger, SwipeType.None,0,Vector2.zero);
		}
		else{
			CreateGesture( fingerIndex, EventName.On_DoubleTap,singleDoubleTap[fingerIndex].finger, SwipeType.None,0,Vector2.zero);
		}
	...
	private void CreateGesture(int touchIndex,EventName message,Finger finger, SwipeType swipe, float swipeLength, Vector2 swipeVector){
		...
		// NGui
		if (enabledNGuiMode){
			finger.isOverGui = finger.isOverGui || IsTouchOverNGui(touchIndex);
		}
	...
	private void TwoFinger(){
		...
		if ( twoFinger.currentGesture==GestureType.None){
			...
			// we notify the touch
			CreateGesture2Finger(EventName.On_TouchStart2Fingers,twoFinger.startPosition,twoFinger.startPosition,twoFinger.deltaPosition,twoFinger.timeSinceStartAction, SwipeType.None,0,Vector2.zero,0,0,twoFinger.oldFingerDistance);				
		}
		...
		if (fingers[twoFinger.finger0].phase != TouchPhase.Ended && fingers[twoFinger.finger1].phase != TouchPhase.Ended && twoFinger.currentGesture != GestureType.Cancel){
			...
			if (twoFinger.currentGesture == GestureType.Acquisition && twoFinger.timeSinceStartAction >= longTapTime && FingerInTolerance(fingers[twoFinger.finger0]) && FingerInTolerance(fingers[twoFinger.finger1])){	
				...
				CreateGesture2Finger(EventName.On_LongTapStart2Fingers,twoFinger.startPosition,twoFinger.position,twoFinger.deltaPosition,twoFinger.timeSinceStartAction, SwipeType.None,0,Vector2.zero,0,0,twoFinger.fingerDistance);				
			}	
			...
			if (move && twoFinger.currentGesture != GestureType.Tap){
				...
				if (enable2FingersSwipe){
					...
					if (dot>0 ){
						...
						if (twoFinger.pickedObject && !twoFinger.dragStart){
							...
							CreateGesture2Finger(EventName.On_DragStart2Fingers,twoFinger.startPosition,twoFinger.position,twoFinger.deltaPosition,twoFinger.timeSinceStartAction, SwipeType.None,0,Vector2.zero,0,0,twoFinger.fingerDistance);	
							...
							CreateGesture2Finger(EventName.On_SwipeStart2Fingers,twoFinger.startPosition,twoFinger.position,twoFinger.deltaPosition,twoFinger.timeSinceStartAction, SwipeType.None,0,Vector2.zero,0,0,twoFinger.fingerDistance);
				...
				if (enableTwist){
					...
					if ( twistAngle>minTwistAngle && (twoFinger.currentGesture != GestureType.Twist ) || twoFinger.currentGesture== GestureType.Twist ){
						...
						CreateGesture2Finger(EventName.On_Twist,twoFinger.startPosition,twoFinger.position,twoFinger.deltaPosition,twoFinger.timeSinceStartAction, SwipeType.None,0,Vector2.zero,twistAngle,0,twoFinger.fingerDistance);
					}
				...
				if (twoFinger.currentGesture == GestureType.LongTap){
					CreateGesture2Finger(EventName.On_LongTap2Fingers,twoFinger.startPosition,twoFinger.position,twoFinger.deltaPosition,twoFinger.timeSinceStartAction, SwipeType.None,0,Vector2.zero,0,0,twoFinger.fingerDistance);
				}
			}
	...
	IEnumerator SingleOrDouble2Fingers(){
		...
		if (singleDoubleTap[99].count <2){
			...
			CreateGesture2Finger(EventName.On_SimpleTap2Fingers,twoFinger.startPosition,twoFinger.position,twoFinger.deltaPosition,
			                     twoFinger.timeSinceStartAction, SwipeType.None,0,Vector2.zero,0,0,twoFinger.fingerDistance);
	...
	private void  CreateGesture2Finger(EventName message,Vector2 startPosition,Vector2 position,Vector2 deltaPosition,
		...
		if (enabledNGuiMode && message == EventName.On_TouchStart2Fingers){
			gesture.isOverGui = gesture.isOverGui || ( IsTouchOverNGui(twoFinger.finger1) && IsTouchOverNGui(twoFinger.finger0));
		}
	...
	private SwipeType GetSwipe(Vector2 start, Vector2 end){
		...
		if ( Vector2.Dot( linear, new Vector2(0.5f,0.5f).normalized) >= swipeTolerance)
			return SwipeType.UpRight;

		if ( Vector2.Dot( linear, new Vector2(0.5f,-0.5f).normalized) >= swipeTolerance)
			return SwipeType.DownRight;
			
		if ( Vector2.Dot( linear, new Vector2(-0.5f,0.5f).normalized) >= swipeTolerance)
			return SwipeType.UpLeft;

		if ( Vector2.Dot( linear, new Vector2(-0.5f,-0.5f).normalized) >= swipeTolerance)
			return SwipeType.DownLeft;
	...
	private bool IsTouchOverNGui(int touchIndex, bool isTwoFingers=false){
		...
		if (enabledNGuiMode){
			...
			while (!returnValue && i<nGUICameras.Count){
				Vector2 pos = Vector2.zero;
				if (!isTwoFingers){
					pos = fingers[touchIndex].position;
				}
				else{
					pos = twoFinger.position;
				}
